...
namespace mqf {
	...
	inline Time DurationFixed( const Gregorian::Date& d1, const Gregorian::Date& d2 ) {
		return Time( (d2.day - d1.day) + (d2.month - d1.month) * daysPerMonth + (d2.year - d1.year) * daysPerYear ) / daysPerYear;
	}

	inline Time Duration( Gregorian::Date d1, Gregorian::Date d2, DayCount<BondBasis_30360> ) {
	...
	inline Time Duration( Gregorian::Date d1, Gregorian::Date d2, DayCount<E_30360> ) {
		d1.day = std::min( d1.day, 30 );
		d2.day = std::min( d2.day, 30 );
		return DurationFixed<30,360>( d1, d2 );
	}

	inline Time Duration( Gregorian::Date d1, Gregorian::Date d2, DayCount<Actual> ) {
